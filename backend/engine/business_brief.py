...
def load_attack_stories() -> dict:
    path = os.path.join(_KB_DIR, "attack_stories.json")
    with open(path) as f:
        return json.load(f)
...
def get_urgency(expected_loss: float) -> dict:
    if expected_loss >= 100000:
        return {
            ...
            "subtitle": "Requires decision this week",
...
def generate_business_brief(result: RiskResult, company: CompanyContext) -> str:
    ...
    for i, step in enumerate(story["steps"]):
        step = (
            step.replace("{records}", records)
            .replace("{data_types}", data_label)
            .replace("{company}", company.company_name)
        )
        steps_text += f"  Step {i + 1}: {step}\n"

    headline = (
        story["headline_template"]
        .replace("{company}", company.company_name)
...
"""
    ...
    if b.regulatory_penalty > 0:
        cost_lines += (
            f"  {f'Regulatory fines ({frameworks}):':<42} {fmt_range(b.regulatory_penalty)}\n"
        )
    if b.reputation_damage > 0:
        cost_lines += (
            f"  {'Lost customers (estimated churn):':<42} {fmt_range(b.reputation_damage)}\n"
        )
...
def generate_executive_summary(results: list, company: CompanyContext, chains: list = None) -> str:
    ...
    unchained_loss = sum(
        r.expected_loss for r in results if r.vulnerability_id not in chained_vuln_ids
    )
    ...
    for i, r in enumerate(results[:3]):
        ...
        gemini_note = (
            " ✓ AI-verified" if (r.gemini_analysis and r.gemini_analysis.is_exploitable) else ""
        )
    ...
    gemini_note = (
        ...
        if any(r.gemini_analysis for r in results)
        else ""
    )
